"""
이메일 인증이 끝나기 전에 정보를 Redis에 임시 저장
"""
...
class PendingSignup:
    ...
    nickname: str
    name: str
    birth: str  # date.isoformat() — Redis hash는 문자열만 저장 가능
    phone: str  # security.encrypt_phone() 결과
    phone_hash: str  # security.make_phone_hash() 결과
...
class SignupPendingStore:
    ...
    async def upsert(self, pending: PendingSignup) -> None:
        ...
        try:
            ...
            if current_nickname and current_nickname.lower() != pending.nickname.lower():
            ...
            raise ExternalServiceException(detail="회원가입 임시 정보 저장에 실패했습니다.") from exc
    ...
    async def get(self, email: str) -> PendingSignup | None:
        try:
            ...
            raise ExternalServiceException(detail="회원가입 임시 정보 조회에 실패했습니다.") from exc
    ...
    async def pop(self, email: str) -> PendingSignup | None:
        ...
        try:
            ...
            raise ExternalServiceException(detail="회원가입 임시 정보 처리에 실패했습니다.") from exc
