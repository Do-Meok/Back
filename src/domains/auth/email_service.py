...
class EmailService:
    ...
    async def send_verification_code(self, to_email: str, code: str, purpose: str) -> None:
        ...
        if self._backend == "smtp":
            await asyncio.to_thread(self._send_smtp_verification_code, to_email, code, purpose)
            return
        raise ExternalServiceException(detail=f"지원하지 않는 이메일 백엔드입니다: {self._backend}")

    def _send_smtp_verification_code(self, to_email: str, code: str, purpose: str) -> None:
        """
        SMTP 통신 담당
        (설정 검증/메일 조립/SMTP 연결 및 발송/예외 변환
        """
        if not self._smtp_host or not self._smtp_from_email:
            raise ExternalServiceException(detail="SMTP 설정이 누락되었습니다.")
        ...
        try:
            ...
            raise ExternalServiceException(detail="이메일 발송에 실패했습니다.") from exc
