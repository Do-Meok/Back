...
@router.get("", status_code=200, summary="장보기 리스트 조회",response_model=list[GetItemResponse])
async def get_list(
        service: ShoppingService = Depends(get_shopping_service),
):
    return await service.get_list()

@router.delete("/{shopping_id}", status_code=204, summary="장보기 리스트 삭제", responses=create_error_response(ItemNotFoundException))
